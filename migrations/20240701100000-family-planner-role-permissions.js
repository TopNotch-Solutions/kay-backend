...
/** Sync family_planner role permissions for Family Planning Suite APIs. */
module.exports = {
  async up(queryInterface) {
    const roleName = ROLES.FAMILY_PLANNER;
    ...
    if (existing.length === 0) {
      await queryInterface.bulkInsert('roles', [{
        name: roleName,
        display_name: AUTHORIZED_CLINIC_ROLES[roleName] || 'Family Planner',
      }]);
    } else {
      await queryInterface.sequelize.query(
        'UPDATE roles SET display_name = :displayName WHERE name = :name',
        {
          replacements: {
            name: roleName,
            displayName: AUTHORIZED_CLINIC_ROLES[roleName] || 'Family Planner',
          },
        }
      );
    }
